...
impl Config {
    ...
    pub fn new<'a, CR: ConfigReader<'a, ConfigItem>>(config_reader: CR) -> Self {
        ...
        for opt in config.config_options {
            if opt.env.is_some() && opt.env != Some(String::from("")) {
                config_option_name = opt.env.clone().unwrap();
            } else if opt.property.is_some() && opt.property != Some(String::from("")) {
        ...
        for unit in config.config_setting.unit {
            ...
            let config_setting_unit_regex = unit.regex.clone().unwrap_or_else(|| "".to_string());
    ...
    pub fn validate(
        ...
        // compare version
        if option.as_of_version.as_str() > product_version {
        ...
        if option.allowed_values.is_some() {
            let allowed_values = option.allowed_values.as_ref().unwrap();
            if !allowed_values.is_empty()
                && !allowed_values.contains(&config_option_value.to_string())
            {
        ...
        T: FromStr + std::cmp::PartialOrd + Display,
    {
        ...
        if config_option_value.is_empty() {
            return Err(ConfigError::ConfigValueEmptyOrNone(
                config_option_name.to_string(),
            ));
        }
